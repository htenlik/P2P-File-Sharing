...
class PeerNode:
    ...
    def register_peer_to_server(self) -> bool:
        ...
        try:
            ...
            Protocol.start_serving(s, self.peer_port)
            reply = Protocol.receive_msg(s)
            print(f"PEER{self.peer_port} is registered to SERVER at {self.server_ip}:{self.server_port} -> {reply}")
            return True
    ...
    def send_file_list_to_server(self) -> bool:
        ...
        if file_list:
            ...
            try:
                s.connect((self.server_ip, self.server_port))
                Protocol.start_providing(s, self.peer_port, file_list)
                reply = Protocol.receive_msg(s)
                return True
            except Exception as e:
    ...
    def execute_schedule(self) -> None:
        ...
        try:
            ...
            try:
                s.connect((self.server_ip, self.server_port))
                Protocol.stop_serving(s, self.peer_port)
                reply = Protocol.receive_msg(s)
                print(f"PEER{self.peer_port} is unregistered from SERVER -> {reply}")
            except Exception as e:
